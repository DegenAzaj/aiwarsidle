...
{
    public sealed class ResumePolicyDebugButton : MonoBehaviour
    {
        [Header("Wiring")]
        [SerializeField] private ResumePolicyController _controller;
        ...
        [SerializeField] private Vector2 _anchoredPosition = new(0f, -10f);
        ...
        private void Start()
        {
            ...
            if (_controller == null)
            {
                _controller = FindObjectOfType<ResumePolicyController>(includeInactive: true);
            }

            if (_controller == null)
            {
                Debug.LogWarning($"{nameof(ResumePolicyDebugButton)}: No {nameof(ResumePolicyController)} found in scene.");
                return;
            }
            ...
            if (canvas == null)
            {
                Debug.LogWarning($"{nameof(ResumePolicyDebugButton)}: No Canvas found in scene.");
                return;
            }
        ...
        private static void EnsureCompatibleInputModule(EventSystem eventSystem)
        {
            ...
            // New Input System only: StandaloneInputModule uses legacy Input and will throw.
...
#else
            // Legacy or Both: keep StandaloneInputModule.
        ...
        private static void EnsureUiActions(EventSystem eventSystem)
        {
            ...
            // Be explicit; some setups don't auto-assign default actions.
        ...
        {
            var go = new GameObject("DEV_InactivityTimeoutButton");
            ...
            button.onClick.AddListener(() => _controller.DebugTriggerInactivityTimeout());
